...
public class UnitTests
{
    ...
    public void TestCalculate()
    {
        ...
        Assert.AreEqual(-22.5m, m_calculator.Calculate("24 - 32 + 29 * 3 / (3 - 9)"));
        ...
        Assert.AreEqual(1_679_625.0m, m_calculator.Calculate("(1+(4+5+2)-3)+(6 ^ 8)"));
    ...
    public void TestDecimalCalculations()
    {
        ...
        Assert.AreEqual(0.3333333333333333333333333333m, m_calculator.Calculate("1 / 3"));
        Assert.AreEqual(0.1428571428571428571428571429m, m_calculator.Calculate("1 / 7"));
        ...
        Assert.AreEqual(5.5m, m_calculator.Calculate("(2.5 + 3) * (4.5 - 3.5)"));
    ...
    {
        Assert.ThrowsException<ArgumentException>(() => m_calculator.Calculate("1 / 0"));
    }

    [TestMethod]
    public void Calculate_EmptyExpression_ThrowsException()
    {
        Assert.ThrowsException<ArgumentException>(() => m_calculator.Calculate(""));
    }
    ...
    {
        Assert.ThrowsException<ArgumentException>(() => m_calculator.Calculate("1 + a"));
    }

    [TestMethod]
    public void Calculate_UnmatchedOpeningParenthesis_ThrowsException()
    {
        Assert.ThrowsException<ArgumentException>(() => m_calculator.Calculate("(1 + 2"));
    }
    ...
    {
        Assert.ThrowsException<ArgumentException>(() => m_calculator.Calculate("1.2.3"));
    }

    [TestMethod]
    public void Calculate_InsufficientOperands_ThrowsException()
    {
        Assert.ThrowsException<ArgumentException>(() => m_calculator.Calculate("1 +"));
    }
}
